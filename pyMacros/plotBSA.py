...
Q2_edges = get_adaptive_edges(tree, "Q2", n_bins=4, min_val=1.0, max_val=6.5)
Xb_edges = get_adaptive_edges(tree, "Xb", n_bins=4, min_val=0.1, max_val=0.7)
t_edges  = get_adaptive_edges(tree,  "t",  n_bins=1, min_val=0.0, max_val=4)
...
print("Adaptive Q2 bin count:", nQ2)
print("Adaptive Xb bin count:", nXb)
print("Adaptive t bin count:", nt)
print("Adaptive phi bin count:", nphi)
...
cols = ["Q2", "Xb", "t", "trentoPhi", "helicity"]
df_numpy = rdf.AsNumpy(columns=cols)

Q2_vals = df_numpy["Q2"]
Xb_vals = df_numpy["Xb"]
t_vals = df_numpy["t"]
phi_vals = (df_numpy["trentoPhi"] + 2*np.pi) % (2*np.pi) * 180.0 / np.pi
helicities = df_numpy["helicity"]
...
# General sine function: A * sin(phi + phi0), phi in degrees
...
# Perform fit with initial guesses: A=0.1, phi0=0
    ...
    for iXb in range(nXb):
        ...
        try:
            ...
            popt, _ = curve_fit(
                ...
                sigma=ALU_proj_err[valid], absolute_sigma=True, p0=[0.1, 0]
            )
...
figTitle = "ALU_vs_phi_grid_Q2_Xb.png"
plt.savefig(figTitle, dpi=300)
plt.show()
